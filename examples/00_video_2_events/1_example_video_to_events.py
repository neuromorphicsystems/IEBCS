...
dvs.set_dvs_sensor(th_pos=th, th_neg=th, th_n=th_noise, lat=lat, tau=tau, jit=jit, bgn=0.1)
dvs.init_thresholds()
...
ed = EventDisplay("Events", cap.get(cv2.CAP_PROP_FRAME_WIDTH), cap.get(cv2.CAP_PROP_FRAME_HEIGHT), 4 * dt, 0)

if cap.isOpened():
    for frame in tqdm(range(10), desc="Converting video to events"):
        ret, im = cap.read()
        if im is None:
            break
        im = cv2.cvtColor(im, cv2.COLOR_RGB2LUV)[:, :, 0]
