...
filename = 'ev_100_10_100_300_0.3_0.05_nonoise.dat'
...
for t in range(ts[0], ts[-1], tw):
    ...
    tsurface[y[ind], x[ind]] = ts[ind]
    indsurface[y[ind], x[ind]] = p[ind]
    ind = np.where(tsurface > 0)
    img[:, :] = 125
    img[ind] = 125 + (2 * indsurface[ind] - 1) * np.exp(-(t + tw - tsurface[ind].astype(np.float32))/ (tw)) * 125
