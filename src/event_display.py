...
class EventDisplay():
    ...
    def update(self, pk, dt):
        ...
        if self.last_frame > self.frametime:
            ...
            if self.render == 1:
                self.im[:, :, 0] = 0.5
                self.im[:, :, 1] = 0.5 + (self.pol_surface - 0.5) * np.exp(-(self.time - self.time_surface.astype(np.double)) / self.render_tau)
                self.im[:, :, 2] = 0.5 + (self.pol_surface - 0.5) * np.exp(-(self.time - self.time_surface.astype(np.double)) / self.render_tau)
